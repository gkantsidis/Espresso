...
#define FFI_EXPORT extern  __declspec(dllexport)
...
FFI_EXPORT char* run_espresso_from_data(const char* data, unsigned int length, char** out) {
    if (length == 0) {
        return NULL;
    }
...
FFI_EXPORT char* run_d1merge_from_data(const char* data, unsigned int length, char** out) {
    if (length == 0) {
        return NULL;
    }
